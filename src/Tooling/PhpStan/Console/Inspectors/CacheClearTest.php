...
class CacheClearTest extends TestCase implements ForInspector
{
    use InspectorTestCases;

    public string $class = CacheClear::class;
    ...
    public array $options {
        get => [
            ['name' => 'configuration', 'configValue' => '/custom/phpstan.neon'],
        ];
    }
    ...
    public function it_includes_phpstan_specific_options(): void
    {
        ...
        $this->assertTrue(
            $optionNames->contains('configuration'),
            'Expected the "configuration" option.'
        );
    }
}
